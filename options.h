...
#ifdef EPOLL
/*
 * epoll 检测玩家缓冲区、是否有信息、及时清理数据
 * 检测时epoll的效率很低、1000人连线效率差十倍以上
 * 效率测试：每人每秒一条命令、单向返回信息的命令、没有广播
 * 不检测玩家缓冲区、广播信息可能保留在玩家缓存区、产生延时
 * select - 1000人 检测 msg_buf CPU 30%
 * epoll  - 1000人 检测 msg_buf CPU 50%
 * epoll  - 1000人 不检测 msg_buf CPU 4%
 * epoll  - 4000人 不检测 msg_buf CPU 16%
 *
 * 广播消息666人每秒广播1条消息666*666、
 * CPU都在40-50%左右、差距不大
 */
#define EP_CHECK_BUF
/* epoll 一次最大处理事件的数目 */
